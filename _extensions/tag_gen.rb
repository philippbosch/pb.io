...
module Jekyll
  ...
  class Site
    ...
    def write_tag_list
      dir = self.config['tag_dir'] || 'tags'
      tags = self.tags.keys.sort!
      tag_list = TagList.new(self, self.source, dir, tags)
      tag_list.render(self.layouts, site_payload)
      tag_list.write(self.dest)
    end
  end
  
  AOP.after(Site, :write) do |site_instance, result, args|
    site_instance.write_tag_indexes
    site_instance.write_tag_list
  end
end
